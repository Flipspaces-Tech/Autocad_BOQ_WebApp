...
function hideEmptyFloorClColumnsAfterC_() {
  const masterSS = SpreadsheetApp.getActiveSpreadsheet();
  const sh = masterSS.getSheetByName(FLOOR_CL_SYNC.TARGET_TAB);
  if (!sh) throw new Error(`Target tab not found: ${FLOOR_CL_SYNC.TARGET_TAB}`);

  const block = findTopRoomMatrixBlock_(sh);

  // After C means start checking from column D
  const startCol = 4;
  const lastCol = block.lastCol;
  if (lastCol < startCol) return;

  // Only inspect data rows, NOT total row
  const numRows = block.totalRow - block.dataStartRow;
  if (numRows <= 0) return;

  const numCols = lastCol - startCol + 1;
  const vals = sh.getRange(block.dataStartRow, startCol, numRows, numCols).getDisplayValues();
  ...
  for (let c = 0; c < numCols; c++) {
    ...
    for (let r = 0; r < numRows; r++) {
      ...
      // numeric check
      const num = toNumber_(raw);

      // if text exists and is not numeric, keep visible
      if (num == null) {
        hasMeaningfulValue = true;
        break;
      }

      // non-zero numeric means keep visible
      if (num !== 0) {
        hasMeaningfulValue = true;
        break;
      }
    ...
    if (hasMeaningfulValue) {
      sh.showColumns(actualCol);
    } else {
      sh.hideColumns(actualCol);
    }
  }
}
function nz_(n) {
...
function syncFloorClToCalculationSheet() {
  ...
  Object.keys(agg).forEach((boqNorm) => {
    ...
    Object.keys(agg[boqNorm]).forEach((zoneNorm) => {
      ...
      // divide right-side value by left-side Carpet Area
      const rawVal = agg[boqNorm][zoneNorm];
      const carpetArea = toNumber_(targetSh.getRange(row, block.colCarpet).getValue());

      if (!Number.isFinite(carpetArea) || carpetArea === 0) {
        targetSh.getRange(row, col).clearContent();
        return;
      }

      const finalVal = round2_(rawVal / carpetArea);
      targetSh.getRange(row, col).setValue(finalVal);
      writes++;
    });
  });

  rebuildTotalsForColsDivided_(targetSh, block.dataStartRow, block.totalRow, affectedCols, block.colCarpet);
  hideEmptyFloorClColumnsAfterC_();
...
function rebuildTotalsForColsDivided_(sh, dataStartRow, totalRow, cols, carpetCol) {
  [...new Set(cols)].forEach((col) => {
    sh.getRange(totalRow, col).setFormula(
      `=IFERROR(SUM(${a1_(dataStartRow, col)}:${a1_(totalRow - 1, col)})/${a1_(totalRow, carpetCol)},0)`
    );
  });
}

/* helpers expected by this script */
function normKey_(s) {
  return String(s || "")
    .toLowerCase()
    .replace(/\u00A0/g, " ")
