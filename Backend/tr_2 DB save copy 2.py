    ...
    try:
        ...
        for k, handle in ed.items():
            try:
                ...
                # XRECORD stores tags like (1, "Visibility1"), (90, 8) etc.
                tags = list(obj.tags)  # list of DXFTag(code, value)

                # Heuristic: whenever we see a string key, look ahead for a numeric value
                for i, t in enumerate(tags):
                    if isinstance(t.value, str):
                        ...
                        # find next numeric-ish value in the next few tags
                        val = None
                        for j in range(i + 1, min(i + 8, len(tags))):
                            v = tags[j].value
                        ...
                        if val is not None:
                            # keep the first hit; don't overwrite unless empty
...
def _iter_child_inserts_in_block(doc, block_name: str):
    """Yield only direct INSERT entities inside a block definition (no recursion)."""
    ...
    for x, y in points:
        ...
        for c in clusters:
            ...
            if (x - cx)**2 + (y - cy)**2 <= tol*tol:
                ...
                c["pts"].append((x, y))
                n = len(c["pts"])
                sx = sum(p[0] for p in c["pts"])
                sy = sum(p[1] for p in c["pts"])
                c["center"] = (sx/n, sy/n)
                placed = True
                break
        if not placed:
            clusters.append({"center": (x, y), "pts": [(x, y)]})
    return clusters

def count_components_for_insert(doc, ins, seat_tol=50.0):
    """
    seat_tol is in DWG units (mm if your file is in mm).
    Tune it: 30–80 usually works.
    """
    parent_name = _ins_name(ins)

    def is_chair(n: str) -> bool:
        n = (n or "").upper()
        return ("CHAIR" in n) or ("SEAT" in n)

    children = list(_iter_child_inserts_in_block(doc, parent_name))
    chair_children = [c for c in children if is_chair(_ins_name(c))]
    use = chair_children if chair_children else children

    pts = []
    for c in use:
        ip = getattr(c.dxf, "insert", None)
        if ip is None:
            continue
        pts.append((float(ip.x), float(ip.y)))

    clusters = _cluster_points(pts, tol=seat_tol)
    breakdown = Counter(_ins_name(c) for c in use if _ins_name(c))
    return 1, len(clusters), breakdown
...
def iter_block_rows(
    ...
    for ins in msp.query("INSERT"):
        try:
            ...
            # If dynamic anonymous, use semantic layer as BOQ name
            if (name or "").upper().startswith("*U"):
                name = (ly or name).strip()
                base_name = name
            ...
            # --- NEW: component counts (nested) ---
            table_cnt, chair_cnt, raw_nested = count_components_for_insert(msp.doc, ins)
            if "ROUND MEETING TABLE" in (_ins_name(ins) or "").upper():
                logging.info("ROUND TABLE children breakdown = %s", dict(raw_nested))


            # ---- NEW: dynamic fallback for chairs ----
            dyn = get_dynamic_props(ins)

            # if nested count failed, try common dynamic property names
            if chair_cnt == 0:
                for key in ("Visibility1", "Chairs", "CHAIRS", "Seats", "SEATS", "NoOfChairs", "NO_OF_CHAIRS", "COUNT"):
                    if key in dyn:
                        try:
                            chair_cnt = int(round(float(dyn[key])))
                            break
                        except Exception:
                            pass


            # 1) TABLE row (always 1 per set)
            out.append(make_row(
                "INSERT", "count", float(table_cnt),
                block_name=parent_name,              # table/set name
                layer=upload_layer,
                ...
                remarks="table (from set)",
                description=desc_txt
            ))

            # 2) CHAIR row (dynamic from nested count)
            if chair_cnt > 0:
                out.append(make_row(
                    "INSERT", "count", float(chair_cnt),
                    block_name=f"{parent_name} - CHAIR",
                    layer=upload_layer,
                    handle="",
                    ...
                    zone=center_zone,
                    category1=(ly or "").strip().lower(),
                    remarks=f"chairs nested in {parent_name}",
                    description=""
                ))
...
def process_one_dxf(
    ...
    if aggregate_inserts:
        ...
        for r in insert_rows:
            ...
            # ensure g is always defined before any use
            g = groups.get(key)
            if g is None:
                g = groups[key] = {
                    "count": 0.0,
            ...
            # qty sum (uses qty_value, not row count)
            try:
